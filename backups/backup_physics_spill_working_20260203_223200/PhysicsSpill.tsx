...
export default function PhysicsSpill({ isActive, shouldReset, boxes, chips }: PhysicsSpillProps) {
  ...
  useEffect(() => {
    ...
    boxes.forEach((box, i) => {
      ...
      // Starting position: spread across top of viewport, above screen
      const startX = (containerWidth / (boxes.length + 1)) * (i + 1)
      const startY = -300 - (i * 50)  // Staggered start heights
    ...
    chips.forEach((chip, i) => {
      ...
      // Starting position: random across top, above screen
      const startX = Math.random() * (containerWidth - 100) + 50
      const startY = -200 - (i * 30) - Math.random() * 200  // Staggered, randomized
